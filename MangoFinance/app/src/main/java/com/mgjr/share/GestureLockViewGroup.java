...
import com.mgjr.R;
import com.mgjr.share.GestureLockView;
...
public class GestureLockViewGroup extends RelativeLayout {
     ...
     * 是否是第一次设置密码
     */
    private boolean isFirstSetGestureLockPwd = true;

    public boolean isFirstSetGestureLockPwd() {
        return isFirstSetGestureLockPwd;
    }

    public void setFirstSetGestureLockPwd(boolean firstSetGestureLockPwd) {
        isFirstSetGestureLockPwd = firstSetGestureLockPwd;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                 ...
                 */
                if (checkAnswer()) {
                    mFingerUpColor = 0xFF378FC9;
                } else {
                    mFingerUpColor = 0xFFFF0000;
                }
                //让每一个gesturebview的画笔颜色也改变
                for (int i = 0; i < mGestureLockViews.length; i++) {
                    mGestureLockViews[i].setmColorFingerUp(mFingerUpColor);
                }

                /*//如果用户链接不足4个点，提示“最少连接4个点，请重新设置”
                if (mChoose.size() < 4) {
                    Toast.makeText(MainApplication.getContext(), "最少连接4个点，请重新设置", Toast.LENGTH_SHORT).show();
                }*/
